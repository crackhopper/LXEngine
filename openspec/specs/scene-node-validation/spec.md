...
The system SHALL provide a high-level `SceneNode` type as the primary `IRenderable` implementation. `SceneNode` construction SHALL require `nodeName` and SHALL initialize a valid renderable shell without requiring constructor-time mesh/material injection. Structural renderable validity SHALL instead depend on the presence of:

- one `MeshComponent`
- one `MaterialComponent`
- zero or one `SkeletonComponent`
- `objectPC`, which SHALL remain present as a transitional member for the engine-wide model push constant

`SceneNode` SHALL be valid outside of any `Scene` container and SHALL perform structural validation immediately once its required structural components are present.

Structural component mutations that change renderable payload (`addComponent<MeshComponent>`, `addComponent<MaterialComponent>`, `addComponent<SkeletonComponent>`, removal of any of those components, or replacement of the underlying mesh/material/skeleton handle through the component API) SHALL trigger immediate re-validation of every currently enabled pass before the mutating call completes.

#### Scenario: independent node validates after required components are attached
- **WHEN** a `SceneNode` is constructed with a valid `nodeName`, then receives a valid `MeshComponent` and `MaterialComponent` while not attached to any `Scene`
- **THEN** the node completes structural validation successfully and exposes pass support based on its validated cache

#### Scenario: structural component mutation revalidates before returning
- **WHEN** the mesh or material bound through a structural component is replaced on an existing `SceneNode`
...
- `SceneNode` construction once the node has the required structural components
- attachment of `MeshComponent`
- attachment of `MaterialComponent`
- attachment or removal of `SkeletonComponent`
- replacement of the mesh/material/skeleton handle through its structural component
- removal of `MeshComponent`
- removal of `MaterialComponent`
...
#### Scenario: node without required structural components cannot support a pass
- **WHEN** `supportsPass(Pass_Forward)` is called on a node that lacks `MeshComponent` or `MaterialComponent`
- **THEN** the method returns `false` and the node does not expose a validated entry for that pass
...
- skinning variant requirements versus `Skeleton` / Bones UBO availability through `SkeletonComponent`

#### Scenario: Missing skeleton for skinned material terminates
- **WHEN** a pass enables a skinning variant that requires `Bones` and the node has no `SkeletonComponent`
...
- **WHEN** a pass enables `USE_SKINNING` and the node lacks `SkeletonComponent` or Bones resources
- **THEN** `SceneNode` logs a `FATAL` structural validation failure for that pass and terminates immediately
