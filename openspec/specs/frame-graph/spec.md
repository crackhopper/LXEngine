...
`LX_core::Scene` SHALL provide `std::vector<IGpuResourceSharedPtr> getSceneLevelResources(StringID pass, const RenderTarget &target) const`. The method SHALL:
...
- `virtual IGpuResourceSharedPtr getUBO() const = 0;` (returning `nullptr` is allowed for lights that do not need a UBO)
