...
- 新建 `src/demos/scene_viewer/scene_builder.{hpp,cpp}`（demo-local glue，不下沉 `infra/`）：`buildHelmetNode(gltfPath) -> SceneNodeSharedPtr`、`buildGroundNode() -> SceneNodeSharedPtr`；内部 `buildMeshFromGltf(loader)` 把 `GLTFLoader` 的 POSITION/NORMAL/TEXCOORD_0/[TANGENT]/indices 拼成 `VertexPosNormalUvBone` buffer + `Mesh`；`makeHelmetMaterial(glTFPbrMaterial, gltfDir)` 基于 `loadGenericMaterial("materials/blinnphong_default.material")` 过渡桥接 baseColorTexture 到 `albedoMap` binding 并设 `enableAlbedo=1`；`enableNormal=0`（DamagedHelmet.gltf 无 TANGENT）
