...
- [x] 1.2 新建 `src/demos/CMakeLists.txt`：`add_subdirectory(lxe_editor)`
- [x] 1.3 新建 `src/demos/lxe_editor/CMakeLists.txt`：`add_executable(lxe_editor main.cpp scene_builder.cpp camera_rig.cpp ui_overlay.cpp)`；链接 `${CORE_LIB}` / `${INFRA_LIB}` / `${GRAPHICS_LIB}` / `imgui`；`target_include_directories(... PRIVATE ${CMAKE_SOURCE_DIR}/src)`；`add_dependencies(... CompileShaders)`

## 2. scene_builder（glTF → 现有材质 glue）

- [x] 2.1 新建 `src/demos/lxe_editor/scene_builder.hpp`：声明 `buildHelmetNode(path)` / `buildGroundNode()`；内部 glue 放匿名 namespace
- [x] 2.2 新建 `src/demos/lxe_editor/scene_builder.cpp` 实现 `buildMeshFromGltf(loader)`：按 `VertexPosNormalUvBone` 拼顶点，TANGENT 缺失时占位 `{1,0,0,1}` + 一次性 warning
...
- [x] 3.1 新建 `src/demos/lxe_editor/camera_rig.hpp`：`class CameraRig { enum class Mode { Orbit, FreeFly }; attach / update / currentMode }`
- [x] 3.2 新建 `src/demos/lxe_editor/camera_rig.cpp`：匿名 namespace 放 `clampUnit` / `syncOrbitFromCamera` / `syncFreeFlyFromCamera`（与 `test_render_triangle.cpp` 的同步公式一致）；`update()` 先 F2 边沿检测 + 切换 pose，再调相应控制器，最后 `camera.updateMatrices()`
- [x] 3.3 构造时 Orbit 默认 target `{0,0,0}` / distance `3.0` / yaw/pitch 0；FreeFly 初始位置 `{0,0,3}`、yaw `180°`、pitch `0`

## 4. ui_overlay（ImGui 注入）

- [x] 4.1 新建 `src/demos/lxe_editor/ui_overlay.hpp`：`UiOverlay { attach(clock, camera, light, rig); drawFrame(); handleHotkeys(input); }`；含 `bool m_prevF1Down` + `bool m_helpVisible=true`
- [x] 4.2 新建 `src/demos/lxe_editor/ui_overlay.cpp`：`drawFrame()` 调 `debug_ui::beginPanel/renderStatsPanel/cameraPanel/directionalLightPanel/endPanel`；`m_helpVisible` 时绘 Help panel（F1/F2/Orbit/FreeFly 键位）
- [x] 4.3 `handleHotkeys(input)` 在 F1 边沿翻转 `m_helpVisible`；F2 交给 `CameraRig`

## 5. main.cpp

- [x] 5.1 新建 `src/demos/lxe_editor/main.cpp`：按 spec 启动顺序；`cdToWhereAssetsExist` 失败 `return 1`；其他 fatal 走 `try/catch` `return 2`
...
- [x] 6.1 新建 `src/demos/lxe_editor/README.md`：Purpose / Upstream requirements / Build & run / Controls / Known limitations 五段齐全
...
- [x] 6.4 Build & Run 具体给出 `cmake --build build --target lxe_editor` + SDL3 LD_LIBRARY_PATH 命令
- [x] 6.5 README 末尾直接复述 7 项手工验收 checklist

## 7. 构建与人工验收

- [x] 7.1 `cmake --build build --target lxe_editor` 通过
