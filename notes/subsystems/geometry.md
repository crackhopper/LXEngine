...
>
> 相关 spec: `openspec/specs/render-signature/spec.md` + `openspec/specs/mesh-loading/spec.md`
...
- `VertexLayoutItem::size` 参与相等比较，但当前不参与 render signature 组装。
- stride 也参与 signature。
- topology 是叶子字符串，不再额外 compose。
- `Mesh::getRenderSignature(pass)` 现在基本不使用 pass，但接口要保留这个参数。
