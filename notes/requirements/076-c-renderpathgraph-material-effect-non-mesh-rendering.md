...
> 原 `REQ-059-a` 重排并改写，2026-06-16 校准到 `REQ-076` 治理与架构调整阶段。当前不再保留“论文实验 sandbox”作为 active 目标；本 REQ 聚焦扩展现有 RenderPathGraph、material、effect 架构，使它能承载 3DGS 等非 mesh 渲染结构，而不是把它们硬塞进 Mesh / SurfaceMaterial。
