...
- Unsupported PBRT features must be reported explicitly, not silently approximated. Exact support or named approximation for `glass`、`fourier`、`mix`、conductor `eta/k` belongs to `REQ-075-a`; this equivalence REQ may compare those sources only after they are marked supported there.

## 依赖

- `REQ-074-i`: OfflineRT config hard cut and smoke。
- `REQ-075-a`: Specialized PBRT BSDF contracts，only when the equivalence profile selects specialized PBRT sources rather than unsupported diagnostics。
