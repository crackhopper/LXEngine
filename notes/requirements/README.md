...
## 当前 active REQ（实施顺序快照，2026-06-16）

2026-06-16 重新校准 active 队列：当前整体仍处于 `REQ-073` 渲染闭环。`REQ-074` 聚焦 BMW M6 场景加载、cache、纹理压缩和 OfflineRT smoke；`REQ-075` 聚焦 OfflineRT / PBRT 质量迭代；`REQ-076` 预留给代码治理、架构调整、文件拆分和核心模块管理；`REQ-077` 支持 3DGS；`REQ-078` 做异步计算优化；`REQ-079` 引入 Temporal/TAA；`REQ-080` 再次治理渲染架构、优化帧率，并补齐 Deferred / Forward+ 等 render path。到 `REQ-080` 完成后，可以把 v0.2.0 游戏引擎渲染器视为初步完善。
...
| `REQ-074-h` | OfflineRT RenderPathGraph Compute Path | OfflineRT graph path |
| `REQ-074-i` | OfflineRT Smoke And Package Readiness Gate | OfflineRT config hard cut / smoke |
| `REQ-075-a` | Specialized PBRT BSDF Contracts | PBRT 高阶材质 source |
| `REQ-075-b` | Offline Realtime Equivalence On New Architecture | 新架构等价验证 |
| `REQ-075-c` | Advanced Offline Path Tracing Reference | Material v3 / OfflineRT 稳定后的完整 path tracing reference |
| `REQ-075-d` | Editor Offline Render Integration | editor 触发、取消、查看离线 job |
| `REQ-076-a` | Large File Decomposition Backlog | 合并 Vulkan realtime / core commands / SceneRuntime 大文件拆分 |
| `REQ-076-b` | Core Module Governance And Human Onboarding | 核心模块治理、review gate、人类接入路径 |
...
| `REQ-079-a` | Temporal Anti-Aliasing Foundation | Temporal frame state、TAA pass、history reset |
| `REQ-080-a` | Renderer v0.2 Performance And Render Path Expansion | v0.2 渲染器性能基线、Deferred / Forward+ 等 render path |
...
- 2026-06-14：删除 active `REQ-072`。原 071 收口审计、legacy hard cut、bindless validation、package 和 equivalence 工作已由后续 active `REQ-073-*`、`REQ-074-*`、`REQ-075-b`、`REQ-076-*` 承接，不再保留独立 active 需求；同步删除 072 专属 Superpowers spec / plan。
