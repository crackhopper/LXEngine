...
- Vulkan IBL bake executor 仍需从 clear-pass 骨架升级为实际 shader bake，用 equirect/cubemap shader 替换当前占位写入。
- GPU irradiance convolution、specular prefilter importance sampling、BRDF LUT bake shader 执行仍待接入。
