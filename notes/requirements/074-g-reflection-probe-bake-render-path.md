...
> 2026-06-15 插入：本 REQ 替换原 `REQ-074-g` post-package cleanup 槽位；原 post-package hard cut 顺延为 `REQ-074-h`。本 REQ 在 `REQ-073-f` 的 environment / skybox 直接光照之后，实现 reflection probe、probe bake cache 和 graph-authored bake render path。
...
- 不把 probe lighting 合成到 Forward/Deferred shader；由 `REQ-075-a` 处理。
- 不实现 parallax-corrected box projection、DDGI、lightmap 或动态 probe update。
- 不解决所有 RenderFeature 参数架构；由 `REQ-076-a` 处理。
