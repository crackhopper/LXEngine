# REQ-034: RenderTarget 拆分为 descriptor 与 binding，引入 MRT / layer / pipeline-key 接入
...
- 立项候选 REQ-035 `FrameGraphResource` 数据结构
...
4. 把 `Camera::matchesTarget` 的语义从 "比单一 RenderTarget" 升级为
   ...
   形状上不被阻塞。
5. 删除遗留 `RenderTarget::getHash` 与对应自测。
...
### R9: 删除 `RenderTarget::getHash` 与对应自测

- 删 `src/core/frame_graph/render_target.hpp` 中的 `usize getHash() const;`
- 删 `src/core/frame_graph/render_target.cpp` 中的实现（若 .cpp 因此变空，
  整个 .cpp 文件可删，让 RenderTarget header-only）
- 删 `src/test/integration/test_pipeline_build_info.cpp` 中
  `testRenderTargetHashStability` 函数及其 `main()` 中的调用
- 这一项可作为前置 cleanup PR 在 REQ-034 实施前先合入，不依赖 REQ-034 任何决策
...
- 删除 `getHash` 后整个工程编译通过、所有现存测试通过

## 修改范围

- `src/core/frame_graph/render_target.hpp` / `.cpp`（拆类型、补字段、删 getHash）
...
待实施。本 REQ 是文档先于代码的设计收口，按 R9 → R1..R8 顺序实施：

1. R9 作为前置 cleanup PR 先合入
2. R1..R8 在一个或一组连续 PR 中完成，每个 PR 必须保持工程可编译可测
3. 落地后重写 `notes/source_analysis/src/core/frame_graph/render_target.md`，
   把"未完成蓝图"叙事换成成型设计的源码导读
