...
> 2026-06-13 更新：本 REQ 原本同时覆盖 GPUResourceTable、bindless、indirect draw、pipeline cache、scene package cache metadata、upload tasks 和 scene load / technique switch loading UI。该范围过大，并且部分术语已经被 RenderPathGraph / RenderPath 取代。后续不再从本文件直接实施；范围已拆分到 `REQ-073-b` 到 `REQ-073-e`、`REQ-074-c` 到 `REQ-074-g`。
...
| RenderPath shader variant 和 `techniques` URI 迁移 | `REQ-073-c` | 在 batching 之前先稳定 final shader reflection、pipeline identity 和 RenderPath 术语边界 |
| indirect draw 覆盖 validation path | `REQ-073-d` | 作为新材质 GPU record / source variant 的 batching 验收，不与 package 格式混在一起 |
| non-bindless / per-material descriptor fallback 禁止 | `REQ-073-e` | 先验证新路径可用，再硬切旧路径并跑 Helmet/BMW realtime smoke |
