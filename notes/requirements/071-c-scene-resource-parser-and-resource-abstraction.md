...
未实施。本文档用于确认 SceneResourceTable parser 拆分、typed storage、resource state/version/dirty propagation、pure envelope override 和 bindless-friendly upload view。旧 `RenderEffectResourceParser` / `Technique` 命名只允许作为历史文件名出现；新实现目标必须迁移到 RenderFeature / RenderPathGraph 语义。
