...
- 已完成受控 realtime/offline 对比场景的当前入口：`assets/scenes/realtime_offline_compare_helmet_pbr.scene.yaml`。早期文档中提到的 `assets/scenes/realtime_offline_compare_flat.scene.yaml` 当前已不存在，不再作为 active 验收资产引用。
