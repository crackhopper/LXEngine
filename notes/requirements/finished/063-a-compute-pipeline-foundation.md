...
因此 3DGS 的 `REQ-077-c` 不应直接承担通用 compute 基建。先做本 REQ，再做 3DGS Vulkan splat pass。
...
- `REQ-077-c`: 3DGS Vulkan splat pass 消费 compute foundation。
- 后续 async compute REQ：独立 compute queue、跨 queue ownership transfer、FrameGraph queue affinity、自动 barrier。

## 实施状态

2026-06-14 复核关闭：通用 compute pipeline foundation 已落地。当前代码包含 `VulkanComputePipeline`、pipeline cache `getOrCreateCompute`、command buffer `vkCmdDispatch` 包装、storage buffer 支持，以及 offline compute renderer / IBL bake / compute shader 相关测试覆盖。

`REQ-077-c` 仍保留 3DGS Vulkan splat pass 的未完成渲染闭环；本文件只归档通用 compute foundation。
