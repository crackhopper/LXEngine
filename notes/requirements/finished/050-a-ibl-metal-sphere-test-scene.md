...
2026-06-14 复核关闭：IBL runtime、HDR environment、cubemap bake 和 PBR/IBL shader 消费路径已经由当前代码保留；但原验收资产 `assets/scenes/ibl_metal_sphere.scene.yaml`、`pbr_ibl` template 和对应 gallery 入口已经从当前资产集删除。

本 REQ 不再作为 active 场景需求维护。后续 IBL/材质验证以当前 `assets/scenes/generated/helmet_standard_pbr.scene.yaml`、`assets/scenes/realtime_offline_compare_helmet_pbr.scene.yaml` 和 Material v3/RenderPathGraph 需求为准。
