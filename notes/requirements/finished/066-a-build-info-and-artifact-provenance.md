...
2026-06-14 复核关闭：BuildInfo 与 artifact provenance 已落地。当前代码包含 `src/infra/build_info/*`、CMake build info 生成、`lxe_editor` API `/build-info`、recording/build metadata、`lxe_offline_render --version` 与输出 sidecar、Node 共享 build-info package、`lxe_manager` / assets-downloader build-info 暴露，以及对应 C++/TS 测试。

本 REQ 不再保留为 active；未来如果需要更完整的 git/compiler provenance，应单独起新需求。
