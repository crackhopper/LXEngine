...
2026-06-14 复核关闭：原 `offlineRender.profiles` / `defaultProfile` / `maxDepth` schema 已不再支持。当前代码使用 `scene.outputProfiles`、`scene.defaultOutputProfile`、`scene.offlineRender.profile` 和 `maxBounce`；`scene_document` 对旧 schema fail-fast。

因此本 REQ 按“旧 schema 已被当前 output profile 合同取代”归档。当前仍未完成的 realtime/offline output profile 工作保留在 `REQ-068-a`。
