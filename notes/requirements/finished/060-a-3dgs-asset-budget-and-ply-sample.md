...
2026-06-14 复核关闭：原计划提交 `assets/models/3dgs_train_sample/point_cloud.ply` 已不符合当前资产策略。当前 3DGS train PLY 不进入 git，通过 assets-downloader cache 管理，示例 URI 为 `cache://voxel51-gaussian-splatting/train_iteration_7000/iteration-7000/converted/point_cloud.ply`。

本 REQ 按“旧 in-git 样例资产策略已废弃”归档；后续 3DGS loader/runtime/render/editor 文档继续在 `REQ-077-a` 到 `REQ-077-e` 中保留，并已改为依赖 cache 资产。
