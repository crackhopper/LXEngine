...
| 兼容路径 | 当前代码表现 | 清理归属 |
|---|---|---|
| OfflineRT 旧入口 | `OfflineRenderJob::offlineShader`、`createOfflineRenderFrameGraph()`、`Pass_OfflineRayTrace` 特判 | `REQ-073-g` / `REQ-073-h` |
| realtime material / descriptor fallback | `DescriptorResourceList` 等价、legacy material validation、旧材质 record 边界 | `REQ-073-e` / `REQ-073-f` |
...
- 不实现 Material v3 source storage、shader variant、indirect batching 或 realtime material hard cut；这些由 `REQ-073-b` 到 `REQ-073-f` 承接。
- 不迁移 `techniques/...` 到 `render_paths/...`；这是 `REQ-073-d`。
- 不删除 OfflineRT 旧 provider / `offlineShader` / hardcoded FrameGraph；这是 `REQ-073-g` / `REQ-073-h`。
...
- 不执行大文件拆分；`src/backend/vulkan/vulkan_realtime_renderer.cpp` 的具体拆分由 `REQ-076-d` 承接。
...
| Bridge | 当前调用点 | 删除归属 |
|---|---|---|
| `OfflineRenderJob::offlineShader` | `src/core/offline/offline_render_job.hpp`、`software_compute_offline_integrator.cpp` | `REQ-073-h` |
| hardcoded offline graph | `src/core/offline/offline_render_work_graph.cpp` | `REQ-073-g` / `REQ-073-h` |
| `Pass_OfflineRayTrace` compute item 特判 | `src/core/frame_graph/render_queue.cpp` | `REQ-073-h` |
...
| old material fallback / non-bindless path | realtime submission and validation boundary | `REQ-073-f` |
...
- OfflineRT 从 hardcoded graph 切到 RenderPathGraph 属于 `REQ-073-g`。
- 删除 `offlineShader` side channel 和 old provider bridge 属于 `REQ-073-h`。
...
- `REQ-073-g` / `REQ-073-h`: OfflineRT RenderPathGraph path and config hard cut。
...
- OfflineRT 仍有 `offlineShader`、hardcoded offline graph 和 `Pass_OfflineRayTrace` 特判，这些由 `REQ-073-g/h` 删除，但本 REQ 要保证它们不污染 renderer foundation/realtime 边界。
