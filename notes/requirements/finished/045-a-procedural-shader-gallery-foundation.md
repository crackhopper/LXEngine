...
2026-06-14 复核关闭：本 REQ 的 procedural material 基础路径已经落地，`proceduralMaterial` scene metadata、runtime 参数流和 scene document round-trip 仍在当前代码中保留。

原第一版 gallery 资产 `rtr_shadertoy_quantum_core*` 与 `assets/scenes/procedural_shader_gallery.scene.yaml` 当前已不存在，后续不再把该 gallery 作为 active 验收资产维护。本文件按“历史功能已完成，示例资产已被后续材质/RenderPathGraph 主线取代”归档。
