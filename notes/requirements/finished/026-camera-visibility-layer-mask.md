...
- `Camera::getCullingMask()` / `setCullingMask(...)` 落地（`src/core/scene/camera.hpp`），默认 `VisibilityMask_All`
- `SceneNode::getVisibilityLayerMask()` / `setVisibilityLayerMask(...)` 落地（`src/core/scene/object.hpp`），默认 `VisibilityMask_All`
- `RenderQueue::buildFromScene` 按命中 pass + target 的 camera OR-combined culling mask 与 renderable `visibilityLayerMask` 交集过滤（`src/core/frame_graph/render_queue.cpp`）
- `Scene` 保持 camera 资源按 target 收集，与 layer 过滤分离
- R1–R5 全部完成
