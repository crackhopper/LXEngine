...
> 2026-06-13 更新：本 REQ 原本同时覆盖 GPUResourceTable、bindless、indirect draw、pipeline cache、scene package cache metadata、upload tasks 和 scene load / technique switch loading UI。该范围过大，并且部分术语已经被 RenderPathGraph / RenderPath 取代。后续不再从本文件直接实施；范围已拆分到 `REQ-073-b` 到 `REQ-073-e`、`REQ-073-j`、`REQ-074-c` 到 `REQ-074-f` 以及 `REQ-074-g`。
...
| non-bindless / per-material descriptor fallback 禁止 | `REQ-073-j` | 先验证新路径可用，再硬切旧路径并跑 Helmet/BMW realtime smoke |
...
REQ-073-h  Offline/realtime render equivalence on the new architecture
```

## 实施状态

2026-06-14 复核关闭：已拆分接管；不从本 REQ 继续实施。本文件移出 active，后续以 `REQ-073-b` 到 `REQ-073-e`、`REQ-073-j`、`REQ-074-e` 和相关后续需求为准。
