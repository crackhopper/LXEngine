...
2026-06-14 复核关闭：IBL GPU bake pipeline 已完成，当前代码包含 `IblBakeRenderer`、IBL bake shader、scene runtime IBL fallback/resource 注入和 `test_vulkan_ibl_bake` 覆盖。
...
- Vulkan framebuffer 测试已覆盖 cubemap face/mip attachment 的实际 GPU 执行：对 `face=3 / mip=1` 开始 color-only render pass、clear 到 HDR float color，再 copy 该子资源到 host buffer 验证非空。
...
- `SceneRuntime` 已提供 CPU 侧 equirectangular HDR -> 方向性 cubemap preview/fallback 路径：`SkyboxMap` 生成 64x64 cubemap，`PrefilteredEnvMap` 按 `roughnessMipCount` 分配 mip chain；VulkanRenderer 会优先执行并消费 GPU bake 产物。

验收补充：

- cubemap face / BRDF LUT 已有 Vulkan readback 非空验证；最终方向与视觉质量由用户在远端截图/目检阶段确认。
