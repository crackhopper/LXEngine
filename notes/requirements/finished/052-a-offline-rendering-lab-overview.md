...
2026-06-14 复核关闭：本文档已经完成 Offline Rendering Lab 的路线锚定作用。当前具体实现和未完成工作已拆到后续独立 REQ：renderer 拆分、Vulkan compute offline renderer、EXR/PNG 输出、path tracing reference、editor integration 和 research sandbox。

本文件不再作为 active 实施单元保留；后续以这些具体 REQ 和当前代码状态为准。
