...
2026-06-14 复核关闭：Vulkan compute offline renderer MVP 已完成。当前代码包含 `src/tools/lxe_offline_render`、`src/backend/vulkan/offline/`、software-compute integrator、`offline_primary_ray.comp`、SceneResourceTable 输入、`SceneSoftwareBvh` 和 `test_offline_gpu_scene` / `test_vulkan_offline_renderer` 覆盖。

原文中引用的早期 `ibl_metal_sphere` 验收场景已不再存在；当前 offline smoke / 对比资产以 generated helmet 和 realtime/offline compare scene 为准。
