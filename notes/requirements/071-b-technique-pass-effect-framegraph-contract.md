...
- `REQ-069-a`：Realtime renderer 拆分有助于接入 FrameGraph executor。
...
未实施。本文档用于确认 RenderPathGraph、RenderPassNode、RenderFeature 和 FrameGraph contract。旧 `Technique` / `RenderEffect` 术语只允许作为历史实现名出现；新实现目标必须使用本文命名和 pure envelope 边界。
