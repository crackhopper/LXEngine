# Assets Downloader（开发中，未完成）：把外部资源整理进本地 Cache
...
> 状态：开发中（未完成）。本工具暂不属于 GetStarted、Tutorial 或 offline ray tracer 的必经路径；当前放在“相关工具”中，用于继续开发、手动验证和资产 cache 方案探索。

它不是 C++ 构建的一部分，也不会在 editor、测试或 offline renderer 启动时自动联网；所有下载都由本地 Web UI 手动触发。
...
第一版 conversion 是整理和登记，不是完整资产烘焙器。复杂归档解压、pbrt scene 转换、完整 glTF 材质拆解和 editor Asset Browser 仍属于后续工作流；当前可验证的是已经落到 `converted/` 下的具体文件。
...
当前 offline software-compute MVP 的主线使用仓库内置 scene 和 assets，不依赖 Assets Downloader。Downloader 后续要解决的是“大文件和 license 不进 git、scene 只保存稳定 URI”的资产管理问题。
...
我们已经把 Assets Downloader 理解为一个开发中的手动联网本地 cache 工作台：它从 catalog 或手动 URL 生成导入计划，把 raw、license、metadata 和 converted 输出写进 `.asset_cache/`。它的目标是服务 scene、editor 和 offline renderer 的稳定 `cache://` URI，但暂不进入教程主线。

## 下一步

继续读 [GetStarted](../get-started.md) 或 [Offline Renderer 教程](../tutorial/offline-renderer/index.md)。当前学习主线不需要启动 Assets Downloader。
