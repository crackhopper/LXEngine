...
pipeline 的结构化身份，底层是一个 `StringID`，由 object/material 两侧 pipeline signature compose 而成。
...
`{shaderName, variants}` 的值类型。enabled variants 会参与 pipeline signature 和 pipeline identity。
