# FrameGraph：把 scene 翻译成按 pass 组织的 RenderWorkItem 列表
...
| 单 pass 收口 | [RenderQueue](render_queue.md) | 一条 pass 内的 RenderWorkItem 过滤、排序、pipeline 去重 |
| 多 pass 调度 | 本页 FrameGraph | 把多条 pass 串起来，跨 pass 全局 PipelineKey 去重 |

读完三页就能回答："一帧渲染的 RenderWorkItem 列表是怎么从 Scene 里挤出来的、
...
- **core/FrameGraph**：决定每条 pass 的 RenderWorkQueue 内容（哪些 RenderWorkItem
  入队 / 哪些 pipeline 需要预构建），并输出依赖排序后的 compiled pass 列表
