...
一个多 pass 材质像一张分步骤菜谱：同一个对象可能先参加 `Shadow`，再参加 `Forward`，以后还可能参加 `GBuffer` 或调试 pass。LXEngine 当前不会把整张菜谱一次性交给 backend，而是把它拆成“某个 pass 下的一份 pipeline work”。
...
`MaterialTemplate` 可以定义多个 pass，但 `RenderWorkItem` 的字段里有明确的 `pass`：
...
`RenderWorkQueue::build(context, pass, target)` 在 realtime domain 下会做三层筛选：
...
多 pass 材质在运行时被拆成多个 pass 视角。`SceneNode` 先把每个 pass 校验成缓存，`RenderWorkQueue` 再按 pass 收集 `RenderWorkItem`，`FrameGraph` 当前按显式 pass 顺序构建这些 queue。真正到 backend 的单位始终是单 pass 的 work item。
