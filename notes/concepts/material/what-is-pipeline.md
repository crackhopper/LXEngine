...
Pipeline 可以先理解成“GPU 厨房的一套固定设备配置”：这套配置决定用哪份 shader、顶点数据怎样解释、三角形怎样剔除、深度怎样测试、颜色怎样混合。多个 work item 如果结构事实完全相同，就可以复用同一套配置。

在 LXEngine 里，pipeline 不是材质本身，也不是 shader 本身。它是一次 `RenderWorkItem` 的材质类型/源码变体与 RenderPath pass contract 被归约后的结果。Realtime raster work 创建 graphics pipeline；offline compute work 创建 compute pipeline。
...
| `shaderVariantKey` | work item 的 shader/material variant |
...
| 关闭某个 pass 或 filter 不命中 | 影响是否产出 item，不改变该 pass 自身 key |
...
如果预构建漏了，运行时 cache miss 会补建并打印 warning；上层仍应尽量从 frame graph / render queue 产物提前收集 pipeline 构建需求。

## 我们已经学会了什么

Pipeline 是 work item 结构的复用单位。当前 `PipelineKey` 由 material type/source variant 与 RenderPathNode signature 组成；`PipelineBuildDesc` 则携带创建 pipeline 所需的 shader、layout、render state、attachment 和 topology 输入。
