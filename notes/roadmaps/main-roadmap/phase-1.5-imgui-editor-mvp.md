...
部分开工。`REQ-035` Transform 组件、`REQ-036` 路径查询、`REQ-037-a` component 模型基础、`REQ-037-b` camera component 接入、`REQ-038-a` ray-AABB picking、`REQ-039-a` DebugDraw 已完成；后续从 `REQ-040-a` 开始继续推进。设计已收口在工作机的临时计划文件 `~/.claude/plans/robust-mapping-flame.md`（不在仓库内），本 phase 文档是其落地索引。
