...
### Frame Graph 演进（REQ-034 + 候选 REQ-035..041）
...
[REQ-034](../../requirements/034-render-target-desc-and-target.md) 把 `RenderTarget` 拆为 `RenderTargetDesc` + `RenderTarget`，是后续 frame graph 演进的 *字段层前置*：attachment 形状要先变得可比、可哈希、能进 `PipelineKey`，才有资格作为资源边的载体。

完整的演进路径（候选 REQ 编号，正式立项时按 `notes/requirements/` 实际可用号顺延）：

| REQ | 内容 | 风险 |
|-----|------|------|
| 035 | `FrameGraphResource` 数据结构 + 跨 pass 资源命名（代码 API） | 低 |
| 036 | `compile()` v1：构边 + 拓扑排序 + cycle 检测 | 低 |
| 037 | 自动 renderpass / framebuffer 创建 | 中 |
| **038** | **自动 barrier 推导** | **高（风险点 #1）** |
| 039 | JSON parser + schema | 中 |
| **040** | **attachment aliasing** | **高（风险点 #2）** |
| 041 | runtime register_pass 策略热替换 | 低 |

**触发条件**：当 [REQ-119](#req-119--g-buffer--延迟渲染路径) 延迟渲染 / [REQ-103](#req-103--shadow-pass--depth-only-pipeline) shadow pass 立项后，跨 pass 资源流转开始变多，REQ-035 应紧随其后立项。在那之前 LX 仍可在"单 forward pass"形态下手工跑，不需要立刻补 frame graph。

**详细调研**：[notes/roadmaps/research/frame-graph/](../research/frame-graph/README.md)（6 篇 + 入口索引）覆盖：完整 frame graph 形态、JSON 资源类型、`FrameGraphResource / FrameGraphNode` 字段拆解、compile 三大算法、LX 字段级 gap、REQ-035..041 切分与里程碑。
...
- async-compute REQ-C 与 frame-graph REQ-035（FramePass 数据结构扩展）共改 `FramePass`，建议同步推进
- async-compute REQ-D（跨 queue 自动 barrier）必须等 frame-graph REQ-038（自动 barrier 推导）完成才能做
...
- 候选 REQ-C 依赖 [`REQ-034`](../../requirements/034-render-target-desc-and-target.md)（RenderTarget 拆分）
- 候选 REQ-E 依赖 frame-graph 候选 REQ-038（barrier 推导） + multi-threading/08 backend 实现
...
- **候选 REQ-F**（核心整合）依赖：REQ-C + REQ-D + REQ-E + gpu-driven REQ-G（mesh shader）+ frame-graph 候选 REQ-038（barrier 推导）+ async-compute 候选 REQ-A（compute pipeline）
...
- REQ-A：frame-graph 候选 REQ-035（FramePass）+ multi-threading/08 IGpuTimeline + RetirePoint
- REQ-D：REQ-A + frame-graph 候选 REQ-038（barrier 推导）+ gpu-driven-rendering 候选 REQ-F（clustered lighting）+ shadows 候选 REQ-F（cubemap shadow）
- REQ-E：REQ-C + REQ-D
...
- REQ-A：frame-graph 候选 REQ-035 + multi-threading/08 IGpuTimeline + RetirePoint
- REQ-D：REQ-A + frame-graph 候选 REQ-038（barrier 推导处理 RT access mask）+ bindless texture
