...
Offline renderer 像一条离线实验流水线：editor 产出场景说明书，loader 把说明书整理进 `SceneResourceTable`，offline `FrameGraph` 编译 pass 依赖，`RenderWorkCompiler` 把样品变成 compute input/desc，Vulkan backend 按统一 pipeline 路径执行，image writer 把结果保存成可复现记录。
...
| Offline pass 创建 | `src/core/offline/offline_render_work_graph.*` | `OutputProfile` | `FrameGraph` + file-local `OfflineCompute` pass |
| Render input 构建 | `src/core/frame_graph/render_work_compiler.*` | `RenderWorkBuildContext::offline(job)` | offline `RenderComputeInput` + `RenderInputDesc` |
...
这条流水线的关键是层次分离：`core` 定义 `SceneResourceTable`、profile、offline pass、storage resources 和 `RenderInputDesc`；`infra` 负责 scene/asset/output 这些文件系统边界；`backend/vulkan/offline` 只负责 headless Vulkan 运行时、graph executor、readback 和 shader 包装。
...
`OfflineSceneLoader` 的任务是“裁剪”：实时 scene 文档包含 editor camera、节点层级、可见性、mesh/material URI、light 和 environment。离线 renderer 不需要 editor 操作状态，也不需要 realtime draw input，所以 loader 使用被选中 `OutputProfile.cameraPath` 找到离线相机，并把可离线计算的字段注册进 `SceneResourceTable`。
...
`software-compute` integrator 现在不直接手写一套独立 dispatch 流程，而是和 realtime 一样先走 `FrameGraph` / `RenderWorkCompiler` 主干。区别在于 offline context 只生成一个 compute input。

| 步骤 | 代码入口 | 当前结果 |
|---|---|---|
| 创建离线 graph | `createOfflineRenderFrameGraph(job.output)` | 一个 file-local `OfflineCompute` pass，target 是 offscreen RGBA16Float |
| 包装 shader | `createOfflineComputeShader("techniques/OfflineRT/offline_pbr_direct_ray")` | `offline_pbr_direct_ray.comp.spv` 被包装成 `IShader` |
| build inputs | `RenderWorkCompiler::buildInputs(pass, context, inputs)` | 生成 `RenderComputeInput`，group count 按 8x8 覆盖输出尺寸，readback resource 是 `OutputPixels` |
| 派生资源 | `buildOfflineSceneStorageResources(job)` | descriptor resources 和 `OutputPixels` storage buffer |
| prepare desc | `RenderWorkCompiler::prepare(pass, context, inputs)` | accepted `RenderInputDesc`，包含 binding plan 和 compute `PipelineBuildDesc` |
| 执行 graph | `OfflineRenderGraphExecutor::execute()` | sync resources、bind pipeline/resources、dispatch、host-read barrier |

这个结构和 [Realtime / Offline 共享流程](../../concepts-design/rendering-pipeline/realtime-offline-shared-flow.md) 对齐：两条入口共享 `RenderInputDesc`、pipeline cache、descriptor 绑定和 command buffer 执行形态，只在输出目标、input kind 和资源形态上分开。
...
`VulkanOfflineRenderer` 是 offline integrator 协调入口。当前只支持 `software-compute`，它初始化 headless `VulkanDevice`，创建 offline `FrameGraph`，把 graph 编译成 compute input/desc，并交给 `OfflineRenderGraphExecutor` 执行。executor 通过 `VulkanResourceManager` 同步 storage buffer、创建或复用 compute pipeline、绑定 descriptor、dispatch `offline_pbr_direct_ray.comp`，最后让 output buffer 可以被 CPU readback。
...
| `FrameGraph` / `RenderInputDesc` | realtime swapchain target |
...
我们已经把 offline renderer 拆成了九段：CLI、scene document、profile、scene loader、offline FrameGraph、RenderInputDesc/storage resources、BVH、compute executor、image writer。每一段都有清晰输入输出，所以后续实现 path tracing 时，可以明确知道应该改哪一层。
