...
新增一种节点 kind 像把新道具引入剧场：道具要写进清单，搬上舞台，能被选中，能被复制，能画辅助线，还要能在演出结束后重新摆回同样位置。当前仓库具备这些基础能力，但还没有统一的 custom node kind 注册入口。

## 一个教学例子：ProbeVolume

我们用 `ProbeVolume` 做例子。它表示一个调试用空间盒子，未来可能用于标记采样区域或可视化区域。
