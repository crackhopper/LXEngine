...
当前代码已经具备 HDR post stack、HDR/cubemap 资源形状、PBR shader 的 scene-level IBL binding 合同，以及一个可加载的 metal sphere scene。运行时已有 CPU 过渡路径把 HDR equirectangular 采样成方向性 cubemap；GPU convolution / prefilter bake 与方向性 skybox 背景仍在 `REQ-048-a` / `REQ-050-a` 的后续切片中推进，因此本教程会把“当前能验证的链路”和“下一步接入的链路”分开讲。
