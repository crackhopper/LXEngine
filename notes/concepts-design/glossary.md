...
| `FrameGraph` | 场景启动阶段的 pass/target/queue 组织骨架。 | [架构总览](architecture.md) |
| `FramePass` | `FrameGraph` 中的一个 pass，包含 pass name、target 和 render queue。 | [材质多 Pass](../concepts/material/pass-rendering-flow.md) |
...
| `MaterialInstance` | 当前唯一运行时材质类型，持有参数、纹理、pass enable 状态。 | [材质系统](../concepts/material/index.md) |
| `MaterialPassDefinition` | 一个 material pass 的 shader、variants、render state 和反射绑定信息。 | [材质多 Pass](../concepts/material/pass-rendering-flow.md) |
| `MaterialTemplate` | 材质蓝图，定义 pass 结构和 canonical material interface。 | [材质系统](../concepts/material/index.md) |
| `ParameterBuffer` | `MaterialInstance` 用来保存 UBO/SSBO 字节数据的 GPU resource 包装。 | [材质系统](../concepts/material/index.md) |
...
| `PipelineBuildDesc` | backend 构建 graphics pipeline 所需的完整输入包。 | [Pipeline 是什么](../concepts/material/what-is-pipeline.md) |
| `PipelineKey` | pipeline cache 使用的结构化身份。 | [模板如何影响 Pipeline](../concepts/material/template-and-pipeline.md) |
...
| `RenderQueue` | 单个 pass 内的 draw queue，把 validated renderables 变成 `RenderingItem`。 | [架构总览](architecture.md) |
| `RenderingItem` | 一次 draw 的 backend 消费上下文。 | [源码分析：Scene](../source_analysis/src/core/scene/scene.md) |
