...
当前 offline software-compute 仍有一个 file-local `OfflineCompute` pass builder，并且 shader 仍通过 `OfflineRenderJob::offlineShader` / provider 进入 compiler preparation；这属于 [REQ-073-g](../../requirements/073-g-offlinert-render-path-graph-compute-path.md) 的后续 OfflineRT graph hard cut，不是旧 queue/item 路径。
...
- [REQ-073-g](../../requirements/073-g-offlinert-render-path-graph-compute-path.md)
