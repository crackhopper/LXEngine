...
`Scene::getSceneLevelResources(pass, target)` 和 `RenderWorkCompiler` 的 scene-renderables input build 会把 target 传给 scene/camera/light 过滤逻辑。当前 target 的职责不是持有 GPU 资源，而是让 scene-level resources 能按输出形状匹配。

| 调用点 | target 的作用 |
|---|---|
| `Camera::matchesTarget` | 判断 camera 是否服务于当前 pass target |
| `Scene::getSceneLevelResources` | 按 pass 和 target 收集 camera/light UBO |
| `RenderWorkCompiler::buildInputs` / `prepare` | 生成并准备带 target identity 的 `RenderInputDesc` |
