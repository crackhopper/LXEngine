...
在 LXEngine 里，这张流程图写在 `assets/render_paths/*.render-path.yaml`。当前默认实时路径包括 Forward / Deferred 及其 bloom 变体；OfflineRT 的 graph asset 仍属于 [REQ-073-g](../../requirements/073-g-offlinert-render-path-graph-compute-path.md) 后续工作。
