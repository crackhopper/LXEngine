...
std::string extensionOf(const LX_core::ResourceUri &uri) {
  ...
  constexpr std::string_view renderPathSuffix = ".render-path.yaml";
  constexpr std::string_view renderFeatureSuffix = ".render-feature.yaml";
  if (path.size() >= renderPathSuffix.size() &&
      path.compare(path.size() - renderPathSuffix.size(),
                   renderPathSuffix.size(), renderPathSuffix) == 0) {
    return std::string(renderPathSuffix);
  }
  if (path.size() >= renderFeatureSuffix.size() &&
      path.compare(path.size() - renderFeatureSuffix.size(),
                   renderFeatureSuffix.size(), renderFeatureSuffix) == 0) {
    return std::string(renderFeatureSuffix);
  }
