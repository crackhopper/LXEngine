...
class GltfMaterialConvertTest(unittest.TestCase):
    ...
    def test_damaged_helmet_conversion_writes_standard_pbr_scene(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("translation: [3.19699, -0.14231, 2.17758]", scene_text)
            self.assertIn("rotation: [0.891314, 0.00777838, 0.453303, -0.00395592]", scene_text)
            self.assertIn("fovY: 35.0", scene_text)
            self.assertIn("kind: Directional", scene_text)
            self.assertIn("direction: [-0.101885, -0.74465, -0.659633]", scene_text)
            self.assertIn("intensity: 4.0", scene_text)
            self.assertIn("rotation: [0.751125, 0.66016, 0.0, 0.0]", scene_text)
