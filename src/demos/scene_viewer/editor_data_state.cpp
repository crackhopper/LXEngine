...
EditorDataDocument EditorDataState::load() const {
  ...
  try {
    ...
    if (document.version != kEditorDataVersion) {
      std::cerr << "[scene_viewer] unsupported editor data version in "
    ...
    std::cerr << "[scene_viewer] failed to load editor data " << m_dataPath
              << ": " << e.what() << "\n";
    return EditorDataDocument{};
  }
...
bool EditorDataState::save(const EditorDataDocument& sourceDocument) const {
  ...
  if (ec) {
    std::cerr << "[scene_viewer] failed to create editor data directory "
  ...
  if (!file) {
    std::cerr << "[scene_viewer] failed to open editor data for write "
              << m_dataPath << "\n";
    return false;
  }
  file << out.c_str();
