...
EditorConfigDocument EditorConfigState::load() const {
  ...
  try {
    ...
    if (document.version != kEditorConfigVersion) {
      std::cerr << "[scene_viewer] unsupported editor config version in "
    ...
    std::cerr << "[scene_viewer] failed to load editor config " << m_configPath
              << ": " << e.what() << "\n";
    return EditorConfigDocument{};
  }
...
bool EditorConfigState::save(const EditorConfigDocument& sourceDocument) const {
  ...
  if (ec) {
    std::cerr << "[scene_viewer] failed to create editor config directory "
  ...
  if (!file) {
    std::cerr << "[scene_viewer] failed to open editor config for write "
              << m_configPath << "\n";
    return false;
  }
  file << out.c_str();
