// REQ-DIAG-MIN-BASELINE: KNOWN-GOOD textbook-style raw-Vulkan demo. This
// file is the verified baseline for resize-bug investigation — confirmed
// stable on the user's hardware across maximize / restore / monitor switch.
// DO NOT MODIFY. If a regression in another demo or the engine needs to
// be reproduced or A/B tested, diff against this file or run
// demo_minimal_resize_baseline alongside the suspect build.
//
// Sibling demo `demo_minimal_resize` is the iteration playground where
// engine wrappers (VulkanDevice / VulkanSwapchain / ...) get folded back
// in step by step; if it ever regresses, the gap from this file localizes
...
private:
  ...
  void initWindow() {
    ...
    m_window = std::make_shared<LX_infra::Window>(
        "demo_minimal_resize_baseline", kWindowWidth, kWindowHeight);
  }
  ...
  void createInstance() {
    ...
    appInfo.pApplicationName = "demo_minimal_resize_baseline";
...
int main() {
  ...
  if (!initializeRuntimeAssetRoot()) {
    std::cerr << "[minimal_resize_baseline] failed to initialize runtime asset root\n";
    return 1;
  }
  ...
  try {
    ...
    std::cerr << "[minimal_resize_baseline] fatal: " << e.what() << "\n";
    return 2;
  }
}
