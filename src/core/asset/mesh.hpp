...
public:
  ...
  using UniquePtr = std::unique_ptr<GeometryStorage>;

  static SharedPtr create(VertexBufferSharedPtr vb, IndexBufferSharedPtr ib) {
    assert(vb && ib);
    return SharedPtr(new GeometryStorage(Token{}, vb->cloneUnique(),
                                         ib->cloneUnique()));
  }
  ...
  [[nodiscard]] UniquePtr cloneUnique() const {
    return UniquePtr(new GeometryStorage(Token{}, m_vertexBuffer->cloneUnique(),
                                         m_indexBuffer->cloneUnique()));
  }
  [[nodiscard]] SharedPtr cloneShared() const {
    return SharedPtr(new GeometryStorage(Token{}, m_vertexBuffer->cloneUnique(),
                                         m_indexBuffer->cloneUnique()));
  }

private:
  GeometryStorage(Token, VertexBufferUniquePtr vb, IndexBufferUniquePtr ib)
      : m_vertexBuffer(std::move(vb)), m_indexBuffer(std::move(ib)) {}

  VertexBufferUniquePtr m_vertexBuffer;
  IndexBufferUniquePtr m_indexBuffer;
};
...
public:
  ...
  using UniquePtr = std::unique_ptr<MeshBuffer>;

  static SharedPtr create(VertexBufferSharedPtr vb, IndexBufferSharedPtr ib,
                          BoundingBox bounds = {}, bool closedVolume = true) {
  ...
  static UniquePtr createRegistered(GeometryStorageHandle storageHandle,
                                    u32 vertexOffset, u32 indexOffset,
                                    u32 vertexCount, u32 indexCount,
                                    BoundingBox bounds = {},
                                    bool closedVolume = true) {
    assert(storageHandle.isValid());
    return UniquePtr(new MeshBuffer(Token{}, storageHandle, vertexOffset,
                                    indexOffset, vertexCount, indexCount,
                                    std::move(bounds), closedVolume));
  }
  ...
  它们不看顶点个数、索引范围、包围盒，也不看具体字节内容。原因是 pipeline
  身份只关心 “这个 draw 需要怎样的 vertex input / primitive assembly
  约束”，而不关心这次画了多少个点。
  ...
  */
  ...
  StringID getPipelineSignature(StringID /*pass*/) const {
    assert(m_storage &&
           "pending mesh storage is required for direct signature");
    return m_storage->getPipelineSignature();
  }

  const VertexLayout &getVertexLayout() const {
    assert(m_storage && "pending mesh storage is required for vertex layout");
  ...
  [[nodiscard]] u32 getVertexCount() const {
    if (m_vertexCount.has_value()) {
      return *m_vertexCount;
    }
    assert(m_storage && "pending mesh storage is required for vertex count");
    return static_cast<u32>(m_storage->getVertexBuffer().getVertexCount());
  }
  [[nodiscard]] u32 getIndexCount() const {
    if (m_indexCount.has_value()) {
      return *m_indexCount;
    }
    assert(m_storage && "pending mesh storage is required for index count");
    return static_cast<u32>(m_storage->getIndexBuffer().indexCount());
  }
  [[nodiscard]] bool isClosedVolume() const { return m_closedVolume; }

  [[nodiscard]] UniquePtr cloneUnique() const {
    if (m_geometryStorageHandle.isValid()) {
      return createRegistered(m_geometryStorageHandle, m_vertexOffset,
                              m_indexOffset, getVertexCount(), getIndexCount(),
                              m_bounds, m_closedVolume);
    }
    return UniquePtr(new MeshBuffer(Token{}, m_storage, m_vertexOffset,
                                    m_indexOffset, m_vertexCount, m_indexCount,
                                    m_bounds, m_closedVolume));
  }

  [[nodiscard]] UniquePtr
  cloneUniqueWithGeometryHandle(GeometryStorageHandle storageHandle) const {
    return createRegistered(storageHandle, m_vertexOffset, m_indexOffset,
                            getVertexCount(), getIndexCount(), m_bounds,
                            m_closedVolume);
  }
...
private:
  ...
  MeshBuffer(Token, GeometryStorageHandle storageHandle, u32 vertexOffset,
             u32 indexOffset, u32 vertexCount, u32 indexCount,
             BoundingBox meshBounds, bool closedVolume)
      : m_geometryStorageHandle(storageHandle), m_vertexOffset(vertexOffset),
        m_indexOffset(indexOffset), m_vertexCount(vertexCount),
        m_indexCount(indexCount), m_bounds(std::move(meshBounds)),
        m_closedVolume(closedVolume) {}
