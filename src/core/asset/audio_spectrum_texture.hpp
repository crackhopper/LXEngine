...
public:
  explicit AudioSpectrumTexture(StringID bindingName = StringID("iChannel0"),
                                u32 width = 512)
      : m_sampler(std::make_shared<CombinedTextureSampler>(
  ...
  void updateFake(float timeSeconds) {
    m_sampler->update(makeFakePixels(m_sampler->texture()->desc().width,
                                     timeSeconds));
  }
  ...
  [[nodiscard]] static std::vector<u8> makeFakePixels(u32 width,
    ...
    for (u32 x = 0; x < width; ++x) {
      ...
      const float high =
          0.5f + 0.5f * std::sin(timeSeconds * 3.9f + u * 37.0f);
