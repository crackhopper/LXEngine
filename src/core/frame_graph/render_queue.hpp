...
/*
@source_analysis.section RenderWorkQueue：一个 pass 内的 draw 列表与 pipeline 收口
RenderWorkQueue 是 per-pass 的，不是全局的 — 每个 `FramePass` 自己持有一个。
之所以做成 per-pass，是因为同一个 renderable 在不同 pass 下产出的 RenderWorkItem
...
RenderWorkItem 共用同一条 pipeline，没必要重复构建。
...
入队前还要把 sceneResources 追加到 RenderWorkItem 的 `descriptorResources` 末尾 —
