...
class VariableResourceTable {
    struct Entry { MetaType meta; u16 refCount; u16 generation; u32 rawOffset; u32 rawSize; };
...
public:
    ...
    [[nodiscard]] ResourceHandle allocate(const MetaType& meta, std::span<const u8> rawData) {
        ...
        if (!m_freeList.empty()) {
            ...
            e.meta = meta; e.refCount = 1; e.generation = 1;
        }
        ...
        ResourceHandle h;
        h.set_all(static_cast<MemoryTypeId>(m_typeId), static_cast<MemoryIndex>(idx), e.generation);
        return h;
    }
    ...
    void release(ResourceHandle handle) {
        ...
        if (e.refCount == 0) { m_rawBuffer.free(e.rawOffset, e.rawSize); m_freeList.push_back(idx); }
    }

    [[nodiscard]] MetaType* getMeta(ResourceHandle handle) {
        if (handle.generation() == 0) return nullptr;
        u32 idx = handle.index();
