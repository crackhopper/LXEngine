...
public:
    ...
    [[nodiscard]] ResourceHandle allocate(const T& dataItem) {
        ...
        ResourceHandle h;
        h.set_all(static_cast<MemoryTypeId>(m_typeId), static_cast<MemoryIndex>(idx), m_metas[idx].generation);
        return h;
    }
    ...
    void release(ResourceHandle handle) {
        ...
        if (m_metas[idx].refCount > 0) m_metas[idx].refCount--;
        if (m_metas[idx].refCount == 0) m_freeList.push_back(idx);
    }
