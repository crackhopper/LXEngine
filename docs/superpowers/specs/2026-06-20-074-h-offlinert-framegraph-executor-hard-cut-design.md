...
Batching semantics are part of the contract. OfflineRT may use
`batching.mode: all` only because its compute shader owns a unified ray tracing
runtime ABI: primitive records, material/ray records, hit shader table, and
`hitShaderIndex` dispatch. Ray visibility metadata such as whether a hit group
casts shadows belongs to that same ray program table and derived primitive
flags, not to node-name or material-name special cases. This must not be reused
as a raster shortcut.
Forward/Deferred raster passes with `batching.mode: material` must group by
material source/layout and bind the matching source-material storage for that
shader variant. A raster material shader must never read mixed-layout
source-material records as one global array.
...
6. Scene participation filtering selects the objects that satisfy the
   ...
   used by draw paths; only the consumed output differs. The batching contract
   still matters: OfflineRT intentionally uses an all-scene compute input, while
   raster passes continue to split by material source/layout.
...
9. OfflineRT scene data preparation follows the resource requirements declared
   ...
   OfflineRT runtime UBO, and the output storage buffer. This global compute
   record set is an OfflineRT shader ABI. It does not permit raster material
   shaders to bind a mixed-layout global source-material buffer.
...
14. `VulkanFrameGraphExecutor` runs in explicit immediate-submit/readback mode.
    ...
    The executor does not reinterpret scene/material schema differently for
    offline than for editor/realtime; differences are limited to target,
    swapchain, and readback/output handling.
...
- hit shader tables: pass-level dispatch tables that assign stable
  `hitGroupIndex` values and ray visibility metadata to allowed material hit
  shader URIs.
...
|---|---|---|---|---|---|
| `hitShaderTable` | RenderFeature YAML | Pass-level table of allowed hit shader programs: stable `index`, material type, hit shader URI, function name, payload contract, and ray visibility metadata such as `castsShadow`. | Stable with the RenderFeature/pass contract; changes when the graph/feature asset changes. | Supplies `hitGroupIndex` constants, primitive flags, and the software dispatch function/switch target. | Supplies hit groups and metadata for future SBT construction, instance masks, or equivalent visibility controls. |
| `PrimitiveHitGroups` | Not authored; derived during pass preparation | Scene-specific lookup from selected primitive to a `hitShaderTable.index` plus lowered primitive flags. | Rebuilt when selected primitives, materials, material hit URIs, or the hit shader table change. | Optional system-owned SSBO only if the software shader reflects that binding. | Lowered into primitive/SBT record association or equivalent backend metadata, not necessarily shader-visible. |
...
- hit shader table entries must have unique `index` values, unique `uri`
  ...
  in this slice;
- `castsShadow` defaults to true when omitted. Entries may explicitly set
  `castsShadow: false`; the compiler lowers that to primitive ray flags so
  shadow rays skip those hit groups while primary/secondary radiance rays can
  still hit them;
