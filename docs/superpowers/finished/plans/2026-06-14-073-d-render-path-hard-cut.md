...
Keep any `techniques/OfflineRT` text only if the sentence explicitly says OfflineRT migration is owned by `REQ-074-h/i`.
...
because OfflineRT migration belongs to `REQ-074-h/i`.
...
- [ ] `assets/shaders/glsl/techniques/OfflineRT` may still exist for `REQ-074-h/i`.
